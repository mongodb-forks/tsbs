...
func createMetricsTable(conf *ClickhouseConfig, db *sqlx.DB, tableName string, fieldColumns []string) {
	...
	sql := fmt.Sprintf(`
			...
			) ENGINE MergeTree() PARTITION BY toYYYYMM(created_date) ORDER BY (tags_id, created_at) SETTINGS index_granularity=8192
			`,
		tableName,
...
func generateTagsTableQuery(tagNames, tagTypes []string) string {
	...
	return fmt.Sprintf(
		"CREATE TABLE tags(\n"+
			...
			"id           UInt32,\n"+
			"%s"+
			") ENGINE MergeTree() PARTITION BY toYYYYMM(created_date) ORDER BY (%s) SETTINGS index_granularity=8192",
		cols,
		index)
}
